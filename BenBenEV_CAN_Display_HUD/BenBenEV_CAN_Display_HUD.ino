...
//                     根据车型修改数据                 //
//-----------------------------------------------------//
#define KM      316   //满电续航，km， 根据车型修改
#define ENERGY  32  //标称电量，kWh，根据车型修改
#define VMax    390   //充满电时的最高电压，根据车型修改
#define VMin    325   //快没电时的最低电压，根据车型修改
...
void setup() {
  ...
  Serial.print(F("DS24(0,240,'energy:       ")); Serial.print(energy/1000.0); Serial.print("kWh',15,0);");
...
void loop()
{
  ...
  if (runtime - brightruntime >= 200)
  {
    bright = brightFilter();
    if (motorspd > -3000)
      spd = abs(motorspd) / 7.12  //7.3170; //←换胎的话修改这个数值 8K的里程偏小，把除数改小一点
    //------------------------------------------------------------------//
    //                            根据个人喜好修改                       //
    //------------------------------------------------------------------//
